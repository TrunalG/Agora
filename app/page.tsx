...
export default function LandingPage() {
  ...
  const submitFeedback = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!feedbackCategory || !feedbackDesc || feedbackRating === 0) return
    setFeedbackStatus('loading')
    try {
      const res = await fetch('/api/feedback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ questionCategory: feedbackCategory, description: feedbackDesc, rating: feedbackRating })
      })
      if (res.ok) {
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground font-sans antialiased select-none">
      ...
      {loaderVisible && (
        <div
          className={`fixed inset-0 z-[60] flex items-center justify-center bg-background text-foreground transition-transform duration-800 pointer-events-none ${!checkingSession ? '-translate-y-full' : 'translate-y-0'
            }`}
      ...
      <div
        className={`min-h-screen transition-all duration-1000 ease-out origin-center ${!isEntranceFinished
          ? checkingSession
      ...
      `}</style>
        ...
        {/* Feedback Section */}
        <section className="py-20 bg-card border-t border-border/50 relative overflow-hidden">
          {/* Subtle Background Glow */}
          <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none opacity-30">
            <div className="w-[600px] h-[300px] bg-primary/20 blur-[100px] rounded-full"></div>
          </div>
          ...
          <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            ...
            <p className="text-muted-foreground text-lg mb-8 max-w-xl mx-auto">
              We are constantly evolving. Share your thoughts, report issues, or suggest new features to make Agora better.
            </p>
            
            <form onSubmit={submitFeedback} className="bg-background/80 backdrop-blur-md border border-border rounded-2xl p-6 sm:p-8 text-left shadow-xl">
              <div className="space-y-5">
                <div>
                  <label htmlFor="feedbackCategory" className="block text-sm font-semibold text-foreground mb-2">
                    What's on your mind?
                  </label>
                  <select
                    id="feedbackCategory"
                    ...
                    className="w-full bg-muted/50 border border-border/80 rounded-lg px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 appearance-none"
                    required
                  >
                    <option value="" disabled>Select a category...</option>
                    <option value="What did you like?">What did you like?</option>
                    <option value="What confused you?">What confused you?</option>
                    <option value="What feature is missing?">What feature is missing?</option>
                    <option value="Report a Bug">Report a Bug</option>
                    <option value="Other thoughts">Other thoughts...</option>
                  </select>
                </div>
                
                <div>
                  <label htmlFor="feedbackDesc" className="block text-sm font-semibold text-foreground mb-2">
                    Description
                  </label>
                  <textarea
                    id="feedbackDesc"
                    ...
                    className="w-full bg-muted/50 border border-border/80 rounded-lg px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none"
                    required
                  ></textarea>
                </div>
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button
                        ...
                        className={`text-2xl transition-transform hover:scale-110 focus:outline-none ${feedbackRating >= star ? 'text-yellow-400' : 'text-muted-foreground/30'}`}
                      >
                        ★
                      </button>
                    ))}
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="w-full h-11 bg-primary text-primary-foreground font-semibold rounded-lg shadow-sm hover:bg-primary/90 transition-all flex items-center justify-center disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {feedbackStatus === 'loading' ? 'Submitting...' : 
                     feedbackStatus === 'success' ? 'Thank You!' : 'Submit Feedback'}
                  </button>
                  {feedbackStatus === 'error' && (
                    <p className="text-red-500 text-xs text-center mt-3 font-medium">Something went wrong. Please try again.</p>
                  )}
                  {feedbackStatus === 'success' && (
                    <p className="text-green-500 text-xs text-center mt-3 font-medium">Your feedback has been submitted to the admin!</p>
                  )}
                </div>
              </div>
            </form>
